...
Originally designed as a final year hardware project, the architecture was upgraded to feature a headless Raspberry Pi Zero 2 W acting as a local web server, bridging wireless HTTP commands to a physical serial-driven kinematics engine.

---

## 🎥 Demonstrations

*(Add a GIF here showing the sideways crab walk)*
...
*(Add a GIF here showing the zero-radius spin)*
...
The physical chassis is stripped down to ensure maximum power delivery to the drive systems:
...
Hosted on the Raspberry Pi Zero 2 W, a lightweight Flask server routes the HTTP requests. It uses the `pyserial` library to convert web commands into raw integer packets, pushing them over a physical USB-OTG connection to the microcontroller at a 115200 baud rate.
...
### Execution
1. Flash the `firmware.ino` to the Arduino Uno and connect it to the Pi via USB-OTG.
